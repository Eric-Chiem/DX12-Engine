...
bool InitD3D()
{
	...
	BYTE* imageData;
	wic imageLoader;
	int imageSize = imageLoader.LoadImageDataFromFile(&imageData, textureDesc, L"braynzar.jpg", imageBytesPerRow);
	...
	// Load Font
	text textLoader;
	arialFont = textLoader.LoadFont(L"Arial.fnt", Width, Height);

	// Load the image from file
	D3D12_RESOURCE_DESC fontTextureDesc;
	int fontImageBytesPerRow;
	BYTE* fontImageData;
	int fontImageSize = imageLoader.LoadImageDataFromFile(&fontImageData, fontTextureDesc, arialFont.fontImage.c_str(), fontImageBytesPerRow);
...
void Update(double delta)
{
	...
	if (Tracking)SetCursorPos(rcClip.left + Width/2, rcClip.top + Height/2);
}
